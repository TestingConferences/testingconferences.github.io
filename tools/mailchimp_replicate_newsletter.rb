...
class MailchimpClient
  def initialize(api_key)
    ...
    raise ArgumentError, 'MAILCHIMP_API_KEY must end with data center suffix (example: -us6)' if dc.nil? || dc.empty?
...
def insert_newsletter_content(original_html, generated_html, placeholder)
  ...
  if original_html.include?(placeholder)
    return original_html.sub(placeholder, generated_html)
  end
...
OptionParser.new do |opts|
  ...
  opts.on('--placeholder TEXT', "Placeholder to replace in template (default: #{DEFAULT_PLACEHOLDER})") { |v| options[:placeholder] = v }
  ...
  opts.on('--dry-run', 'Do not call Mailchimp APIs; print generated HTML preview only') { options[:dry_run] = true }
end.parse!
...
filtered_events = raw_events.select do |event|
  end_date = parse_end_date(event['dates'])
  end_date && end_date >= today && end_date <= cutoff
end

selected_events = filtered_events.first(options[:limit])
...
if source_id.empty?
  warn 'Missing source campaign ID. Use --source-campaign or set MAILCHIMP_SOURCE_CAMPAIGN_ID.'
  exit 1
end
...
if options[:subject]
  client.request('PATCH', "/campaigns/#{new_campaign_id}", body: { settings: { subject_line: options[:subject] } })
end

title = "#{options[:title_prefix]} #{today}"
client.request('PATCH', "/campaigns/#{new_campaign_id}", body: { settings: { title: title } })
